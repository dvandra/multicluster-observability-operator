...
// This is intentionally used to make hub/right-sizing work even when policy propagation to local-cluster
// is disabled (e.g. hub self-management off), while keeping the same rule content.
func ApplyPrometheusRule(ctx context.Context, c client.Client, desired monitoringv1.PrometheusRule) error {
	...
	// Preserve immutable metadata from the existing object, but overwrite spec/labels/annotations to match desired.
