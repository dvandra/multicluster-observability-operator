...
# ─────────────────────────────────────────────────────────────────────
# Import GPU right-sizing mock data into an OpenShift Prometheus.
#
# What it does (end-to-end, no local promtool required):
#   1. Generates 5 days of synthetic GPU metrics in OpenMetrics format.
...
#
# Usage:
#   ./import-gpu-mock-to-ocp.sh            # 5 days (default), openshift-monitoring
#   ./import-gpu-mock-to-ocp.sh 10         # 10 days
#   PROM_NAMESPACE=my-ns PROM_POD=prom-0 ./import-gpu-mock-to-ocp.sh
#
...
GEN_SCRIPT="${SCRIPT_DIR}/generate-gpu-mock-data.py"
MOCK_RULE="${SCRIPT_DIR}/gpu-base-metrics-mock-prometheusrule.yaml"
TMP_FILE="/tmp/gpu-mock-$$.om"
...
REMOTE_TMP="${DATA_DIR}/gpu-mock.om" # must be on the writable PVC (rootfs is read-only)
...
info "Generating ${DAYS} day(s) of GPU mock metrics…"
...
# ── Done ─────────────────────────────────────────────────────────────
ok "GPU right-sizing mock data (${DAYS}d history) imported into ${NS}/${POD}."
